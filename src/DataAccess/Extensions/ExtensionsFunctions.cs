...
namespace AdvancedLogging.Extensions
{
    ...
    public static class ExtensionsFunctions
    {
        ...
        /// </summary>
        public static void PerformRetryDelay(AutoLogFunction logFunction, int retryWaitMS)
        {
            if (retryWaitMS > retryMessageAfterWaitMS)
            {
                var endTime = DateTime.Now.AddMilliseconds(retryWaitMS);
                logFunction.WriteLog($"{logFunction.Name}: Waiting {retryWaitMS / 1000} seconds before retrying...");
        ...
        /// </summary>
        public static async Task PerformRetryDelayAsync(AutoLogFunction logFunction, int retryWaitMS)
        {
            if (retryWaitMS > retryMessageAfterWaitMS)
            {
                var endTime = DateTime.Now.AddMilliseconds(retryWaitMS);
        ...
        /// </summary>
        public static async Task PerformRetryDelayAsync(AutoLogFunction logFunction, int retryWaitMS, CancellationToken cancellationToken)
        {
            if (retryWaitMS > retryMessageAfterWaitMS)
            {
                var endTime = DateTime.Now.AddMilliseconds(retryWaitMS);
        ...
        /// </summary>
        public static void HandleException(ICommonLogger logger, ILoggingContext loggingContext, string functionName, AutoLogFunction vAutoLogFunction, Exception ex, int attempt, int maxRetries, ref bool success, ref int timeoutIncrement, int autoTimeoutIncrement)
        {
        ...
        public static T ExecuteWithRetry<T>(ICommonLogger logger, ILoggingContext loggingContext, Func<T> action, string actionName, AutoLogFunction vAutoLogFunction, int retries, int retryWaitMS, int autoTimeoutIncrement)
        {
            bool success = true;
            T result = default;
            for (int i = 0; i < retries + 1; i++)
            {
                ...
                {
                    HandleException(logger, loggingContext, actionName, vAutoLogFunction, ex, i, retries, ref success, ref timeoutIncrement, autoTimeoutIncrement);
                }
                catch (Exception ex)
                {
                    HandleException(logger, loggingContext, actionName, vAutoLogFunction, ex, i, retries, ref success, ref timeoutIncrement, autoTimeoutIncrement);
                }
                PerformRetryDelay(vAutoLogFunction, retryWaitMS);
            }
            return result;
        }
    }
